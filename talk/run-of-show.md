...
| 4   | Act one: the data tier (Prompt 1)       | 4:00   | 10:30   | The agent turns the data model in PRD section 4 into DDL, compares it with the reference schema, deploys the sandbox on 3310, runs the DDL over MCP, and loads the fixture as the data contract. Call out the MariaDB idioms as they land, and name what they mean: this is current MariaDB, not MySQL, not a stale snapshot. Be plain that the spec asks for them; the proof is the server accepting the DDL and 61 notes loading. The velocity beat. |
...
| 6   | Act three, optional: the API tier (Prompt 3) | 4:00 or 0:30 | 13:30   | Full act: the same context puts a REST Service in front of the schema, one statement per session, and `SHOW REST` shows the endpoints are defined. If the run trips on the session rule, the agent recovers on its own, the clearest proof that skills close the loop. Then it adds REST mode to the app, which starts, reports the missing router, and stays up. Skip path: show the recorded `SHOW REST VIEWS` output and say the boundary line. Either way, one sentence on the boundary: serving the endpoints over HTTP is a router away, out of scope. Anchors 1 and 3. |
...
1. **Recorded run, narrated (primary).** A screen capture of the last clean full run in two cuts. The core cut covers acts one and two, trimmed to the schema landing and the app opening. The optional-act cut covers the REST break-and-recover (if the run had one), the `SHOW REST` metadata, and the two modes. Narrate over it live. Keep it local on the laptop, never streamed.
